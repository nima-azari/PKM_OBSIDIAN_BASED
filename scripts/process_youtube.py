...
def process_youtube_links(links_file: str = "data/sources/youtube_links.txt", convert_to_article: bool = False):
    ...
    for idx, url in urls_to_process:
        ...
        try:
            ...
            print(f"  ✓ SUCCESS - Saved to: {note_path}")
    ...
    if processed_indices:
        print(f"\n" + "="*60)
        print(f"✅ Successfully processed {len(processed_indices)} videos")
        print("="*60)
        print(f"\nCommenting out processed URLs in {links_file}...")
    ...
    print(f"\n📁 OUTPUT DIRECTORY: data/sources/")
    print(f"\nNext steps:")
    print(f"1. Review transcripts in data/sources/")
    print(f"2. Build graph: python build_graph.py")
    print(f"3. Chat with content: python server.py")
    print("\n")
